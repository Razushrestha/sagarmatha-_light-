...
export default function PurchaseReturnsPage() {
  const [returns, setReturns] = useState<Array<{
    ...
    supplier?: { name: string }; originalPurchase?: { invoiceNumber: string };
  ...
  return (
    <DashboardLayout>
      ...
      <div className="card overflow-x-auto">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-brand-50">
            {returns.length === 0 ? (
              <tr><td colSpan={6} className="table-cell text-center py-12 text-gray-400">No purchase returns yet</td></tr>
            ) : returns.map((r) => (
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title="Process Purchase Return" size="lg">
        <form onSubmit={handleSubmit} className="form-modal">
          <FormField label="Original Purchase" required>
            <SelectField value={selectedPurchase} onChange={selectPurchase} placeholder="Select purchase..."
              options={purchases.map((p) => ({
                value: p._id,
                label: `${p.invoiceNumber} ${p.supplier?.name || ""} ${formatCurrency(p.total)}`,
              }))} />
          </FormField>
